...
{
/*
	if (!isset ($config['annu_version']) || $config['annu_version'] != '1.0.0' || $this->installed_version != $mod_config['mod_version'])
	{
		return;
	}
*/
...
class install_convert extends module
{
	...
	function main($mode, $sub)
	{
		...
		switch ($sub)
		{
			case 'intro':
				...
				if(!$this->p_master->installed_version)
				{
					$template->assign_vars(array(
						'S_NOT_INSTALLED'		=> true,
						'TITLE'					=> $lang['BOARD_NOT_INSTALLED'],
						'BODY'					=> sprintf($lang['DIRECTORY_NOT_INSTALLED_EXPLAIN'], append_sid($phpbb_root_path . 'install/index.' . $phpEx, 'mode=install&amp;language=' . $language)),
					));
				...
				if ($new_conversion)
				{
					$config['dir_convert_progress'] = '';
					$config['dir_convert_db_server'] = '';
					$config['dir_convert_db_user'] = '';
					$db->sql_query('DELETE FROM ' . CONFIG_TABLE . "
						WHERE config_name = 'dir_convert_progress'
							OR config_name = 'dir_convert_db_server'
							OR config_name = 'dir_convert_db_user'"
					);
				}

				// Let's see if there is a conversion in the works...
				$options = array();
				if (!empty($config['dir_convert_progress']) && !empty($config['dir_convert_db_server']) && !empty($config['dir_convert_db_user']) && !empty($config['dir_convert_options']))
				{
					$options = unserialize($config['dir_convert_progress']);
					$options = array_merge($options, unserialize($config['dir_convert_db_server']), unserialize($config['dir_convert_db_user']), unserialize($config['dir_convert_options']));
				}
	...
	function list_convertors($sub)
	{
		...
		while ($entry = readdir($handle))
		{
			if (preg_match('/^convert_([a-z0-9_]+).' . $phpEx . '$/i', $entry, $m))
			{
				...
				{
					$sort[strtolower($convertor_data['directory_name'])] = sizeof($convertors);

					$convertors[] = array(
						'tag'				=>	$m[1],
						'directory_name'	=>	$convertor_data['directory_name'],
		...
		foreach ($sort as $void => $index)
		{
			$template->assign_block_vars('convertors', array(
				...
				'SOFTWARE'	=> $convertors[$index]['directory_name'],
	...
	*/
	function get_convert_settings($sub)
	{
		...
		{
			$connect_test = false;
			$available_dbms = get_available_dbms(false, true, false);
			...
			if (!sizeof($error))
			{
				...
				if (!$result)
				{
					...
					{
						$msg .= sprintf($lang['DEFAULT_PREFIX_IS'], $convertor_data['directory_name'], $convertor_data['table_prefix']);
					}
			...
			if (!sizeof($error))
			{
				...
				set_config('dir_convert_progress', serialize(array(
					'step'			=> '',
					'table_prefix'	=> $src_table_prefix,
					'tag'			=> $convertor_tag,
				)), true);
				set_config('dir_convert_db_server', serialize(array(
					'dbms'			=> $src_dbms,
					'dbhost'		=> $src_dbhost,
					'dbport'		=> $src_dbport,
					'dbname'		=> $src_dbname,
				)), true);
				set_config('dir_convert_db_user', serialize(array(
					'dbuser'		=> $src_dbuser,
					'dbpasswd'		=> $src_dbpasswd,
				)), true);

				// Save options
				set_config('dir_convert_options', serialize(array('refresh' => $refresh)), true);
...
//					'S_HIDDEN'	=> $s_hidden_fields,
	...
	*/
	function convert_data($sub)
	{
		...
		if (isset($config['dir_convert_progress']))
		{
			$convert->options = unserialize($config['dir_convert_progress']);
			$convert->options = array_merge($convert->options, unserialize($config['dir_convert_db_server']), unserialize($config['dir_convert_db_user']), unserialize($config['dir_convert_options']));
		}
...
//				$percentage = ($skip_rows == 0) ? 0 : floor(100 / ($total_rows / $skip_rows));
	...
	*/
	function sync_dir($sync_batch)
	{
		...
		$template->assign_block_vars('checks', array(
			'S_LEGEND'	=> true,
			'LEGEND'	=> $user->lang['SYNC_LINKS'],
		));

		$batch_size = $convert->batch_size;

		$sql = 'SELECT MIN(link_id) as min_value, MAX(link_id) AS max_value
			FROM ' . DIR_LINK_TABLE;
		...
		while ($sync_batch <= $primary_max && still_on_time())
		{
			...
			sync_cats('links', 'link_id BETWEEN ' . $sync_batch . ' AND ' . $end);

			$template->assign_block_vars('checks', array(
				'TITLE'		=> sprintf($user->lang['SYNC_LINK_ID'], $sync_batch, ($sync_batch + $batch_size)) . ((defined('DEBUG_EXTRA') && function_exists('memory_get_usage')) ? ' [' . ceil(memory_get_usage()/1024) . ' ' . $user->lang['KIB'] . ']' : ''),
				'RESULT'	=> $user->lang['DONE'],
			));
	...
	*/
	function finish_conversion()
	{
		...
		$db->sql_query('DELETE FROM ' . CONFIG_TABLE . "
			WHERE config_name = 'dir_convert_progress'
				OR config_name = 'dir_convert_options'
				OR config_name = 'dir_convert_db_server'
				OR config_name = 'dir_convert_db_user'");
		...
		add_log('admin', 'LOG_INSTALL_CONVERTED', $convert->convertor_data['directory_name'], $config['version']);
	...
	*/
	function final_jump($final_jump)
	{
		...
		if ($final_jump == 1)
		{
			parse_links_descriptions();

			$template->assign_block_vars('checks', array(
				'TITLE'		=> $user->lang['LINK_DESC_CLEAN'],
				'RESULT'	=> $user->lang['DONE'],
			));
	...
	*/
	function jump($jump, $last_statement)
	{
		...
		if ($jump == 1)
		{
			...
			parse_cats_descriptions();

			$template->assign_block_vars('checks', array(
				'TITLE'		=> $user->lang['CAT_DESC_CLEAN'],
				'RESULT'	=> $user->lang['DONE'],
			));
	...
	*/
	...
	'src_dbms'			=> array('lang' => 'DBMS',			'type' => 'select', 'options' => 'dbms_select(\'{VALUE}\', false)', 'explain' => false),
